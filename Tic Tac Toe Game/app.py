...
colorofline = (0,0,0)
backgroundcolour = (128, 254, 0)
colorofcircle= (0, 0, 255)
xcolor = (255,0,0)
player = 0
gameover = False
...
# Initialize pygame
pygame.init()
pygame.display.set_caption("TIC TAC TOE GAME by Arshiya")
...
def win(player):
    return verticalwin(player) or horizontalwin(player) or diagonalwin(player)
...
def drawverticalline(col, player):
    ...
    color = colorofcircle if player == 1 else xcolor
    pygame.draw.line(Screen, color, (posX, 10), (posX, heightofscreen - 10), circlelinewidth)
 
def drawhorizontalline(row, player):
    posY = row * sizeofsquare + sizeofsquare/ 2
    color = colorofcircle if player == 1 else xcolor
    pygame.draw.line(Screen, color, (10, posY), (widthofscreen - 10, posY), circlelinewidth)
 
def drawdiagonalline(player, down_diag=True):
    color = colorofcircle if player == 1 else xcolor
    if down_diag:
        pygame.draw.line(Screen, color, (25, 25), (widthofscreen - 25, heightofscreen - 25), xlinewidth)
    else:
        pygame.draw.line(Screen, color, (25, heightofscreen - 25), (widthofscreen - 25, 25), xlinewidth)
